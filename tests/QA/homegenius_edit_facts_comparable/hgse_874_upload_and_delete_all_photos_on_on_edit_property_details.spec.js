const { test,expect, launch, faker } = require("../../../lib/qawHelpers");
...
test("hgse_874_upload_and_delete_all_photos_on_on_edit_property_details", async () => {
 // Step 1. HGSE-874 - Upload photos on on edit property details and close modal to assert photos still there
  ...
  const chosenFiles = []
  //const randomUploadNumber = faker.datatype.number({ min: 6, max: filePaths.length });
  const randomUploadNumber = Math.floor(Math.random() * (filePaths.length - 6) + 6);
  ...
  // Click the "X" button
  await page.locator(`div:has(>p:has-text("${property.addressLineOne}")) + div span:text-is("close")`).click({delay:1000})
  
  //--------------------------------------Step 5------------------------------------
  //--------------------------------------------------------------------------------
  
  // Click "Confirm"
  await page.locator(`button:text-is("Continue Editing") + button:text-is("Close"):visible`).click({delay:1000})
  ...
  await expect(page.locator(`[id="photoinput"] li`)).toHaveCount(randomUploadNumber)
  ...
  // Click on Property Type Dropdown and Click Single Family
  await page.locator(`#select-input-container-div-propertyType`).click();
  ...
  // Click "Calculate"
  await page.locator(`button:has-text("Calculate")`).click({delay:3000});
  // ---------------------
  // Assert 
  // ---------------------
  // Wait for the modal to dissapear
  await expect(page.locator(`:text("Processing updates and calculating mygeniusprice.")`)).not.toBeVisible({timeout:80_000})
  await expect(async () => {
  ...
  console.log(amount)
    ...
    // Define a regex pattern to match a dollar sign followed by a number
    const dollarNumberRegex = /\$\d[\d,]*/;
