...
test("hgse_1806_price_filter_logic_for_sold_listings", async () => {
 // Step 1. HGSE-1806: Price Filter Logic for Sold listings
  ...
  // Filter by price
  await page.getByText(`Recently Updatedexpand_more`).click();
  ...
  //     const href = await page.locator(`[data-testid="undecorate"]:has-text("$") >> nth=0 >> a`).getAttribute('href');
  ...
  while(iterariton<numPages) {
    ...
    for (let i=0; i<propertyCount; i++) {
      ...
      const isPriceVisible = await page2.locator(`div:has(div:has-text("Off Market")) + div p:has-text("${listPrice}")`).first().isVisible()
      const isPriceSQFtVisible = await page2.locator(`div:has(div:has-text("Off Market")) + div p:has-text("${listPrice}") + p`).first().isVisible()
      // If we see a page with "No data available" continue with the next page
      if(await page2.locator(`p:has-text("No Data Available")`).isVisible() || !price || !isPriceVisible || !isPriceSQFtVisible) {
      ...
      // Expected to see ($--/sqft) based on new story HGE-2092
      if (await page2.locator(`p:text("($--/sqft)")`).isVisible()) {
        // Close the tab
        await page2.close()
        await page.bringToFront()
      ...
      await expect(page2.locator(`div:has(div:has-text("Off Market")) + div p:has-text("${listPrice}") + p`)).toBeVisible()
 ...
 // Step 2. Price Filter Logic for "For Sale" Listings
  ...
  // Click the "Sold" option'
  await page
    .locator(`button:has-text("Sold"):has(span:text-is("expand_more"))`)
    .click();
  ...
  // unselect land checkbox
  await page.getByRole(`checkbox`, { name: `Land` }).click();
  ...
  while (iterariton < numPages) {
    ...
    for (let i = 0; i < propertyCount; i++) {
      ...
      // Grab the list price
      const listPrice = await propertyCardLocator
        .nth(i)
        .locator(`div.card-media-container + div > div:has-text("$")`)
        .innerText();
      const price = parseInt(listPrice.replace(/\D/g, ""));
      // Go to the URL
      const page2 = await context.newPage();
      ...
      if (
        (await page2.locator(`p:has-text("No Data Available")`).isVisible()) ||
        !price
      ) {
        // Close the tab
        await page2.close();
      ...
      // Check if the property status is "Active" or "Pending" and the price matches the listPrice
      const activeLocator = page2.locator(
        `div:has(div:has-text("Active")) + div p:has-text("${listPrice}")`,
      ).first();
      const pendingLocator = page2.locator(
        `div:has(div:has-text("Pending")) + div p:has-text("${listPrice}")`,
      ).first();
      const comingSoonLocator = page2.locator(
        `div:has(div:has-text("Coming Soon")) + div p:has-text("${listPrice}")`,
      ).first();
      ...
      if (!isActiveVisible && !isPendingVisible && !isComingSoonVisible) {
        throw new Error(
          "Property is neither Active nor Pending nor Coming Soon with the specified price",
        );
      }
  
      // Commment out below due to  new story HGE-2092
      // // Grab the price per sq ft
      // const priceSQFtString = await page2
      //   .locator(`p:has-text("${listPrice}") + p`)
      //   .innerText();
      // const pricePerSQFt = parseInt(priceSQFtString.split("/")[0].slice(2));
      ...
      // // Grab the sq ft for the house and convert to numbers
      // const sqFtString = await page2
      //   .locator(`span:has-text("square_foot") + p`)
      //   .innerText();
      // const sqft = parseInt(sqFtString.replace(/\D/g, ""));
      ...
      try {
        console.log(
          "Checking if the property has the 'Active' history listing in the 'Property History' Section",
        );
  
        // // Scroll dowwn to the "Property History" table
        // await page2
        //   .locator(`#PropertyHistoryTableHeader`)
        ...
        for (const status of statuses) {
          const locator = page2.locator(
            `#PropertyHistoryTableHeader + div:has-text("${status}")`,
          );
          if (await locator.isVisible()) {
            isVisible = true;
            break;
          ...
          try {
            ...
            await Promise.any(
              statuses.map((status) =>
                page2
                  .locator(
                    `[id^="PropertyDetailsTransactionLine-"]:has-text("${status}"):has-text("${listPrice}")`,
                  )
                  .first()
                  .isVisible(),
              ),
      ...
      if (thrwError) {
        throw new Error(
          "Price history value does NOT match the home value shown for ACTIVE listings. Price of: " +
            listPrice,
        );
      }
    ...
    await page
      .locator(
        `button:has-text("chevron_left") + p + button:has-text("chevron_right")`,
      )
      .click();
