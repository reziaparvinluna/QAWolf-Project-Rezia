...
test("hgse_463_mobile_and_tablet_responsive_saved_search", async () => {
 ...
 // Step 3. Delete Saved Searches
  ...
  await expect(editE1).toHaveScreenshot(`editE01.png`, {
 ...
 // Step 4. Verify Saved searches UI on iphone 14
  ...
  if(await page1.getByRole('button', { name: 'Accept Cookies'}).count() > 0) {
  ...
  await expect(page1.locator(`[href$="consumer-account/claimed-homes"]`)).toBeVisible();
  ...
  await expect(page1.locator(`[href$="consumer-account"]`)).toBeVisible();
  ...
  await page1.getByText(`Name Your SearchName Your`).click();
  await page1.getByLabel(`Name Your Search`).fill(savedSearchName);
