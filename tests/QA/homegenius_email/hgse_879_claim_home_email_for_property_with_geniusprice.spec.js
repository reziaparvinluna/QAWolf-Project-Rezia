...
test("hgse_879_claim_home_email_for_property_with_geniusprice", async () => {
 // Step 1. HGSE-879 - Claim home email for property with geniusprice
  ...
  const searchAddress = {
    searchAddress: "32 West Avenue, Arkport, NY 14807",
    searchAddr2: "32 West Ave",
    addressLineOne: "32 West Ave",
    addressLineTwo: "Arkport, NY 14807",
    addressAssert: "32 West Ave",
    addressAssert2: "ARKPORT, NY 14807",
  }
  ...
  await expect(emailPage.getByText("You Claimed a Home")).toBeVisible();
  await expect(emailPage.getByText(searchAddress.searchAddr2)).toBeVisible();
  await expect(emailPage.getByText(searchAddress.addressAssert2)).toBeVisible();
  ...
  await expect(
    emailPage.getByText(
      `Real estate services provided by homegenius Real Estate LLC and homegenius Real Estate Inc. (collectively dba homegenius Real Estate)`
    )
  ).toBeVisible();
  ...
  await page.reload();
  await expect(page.locator(`#CLAIMED_VIEW_PRICES div`).filter({ hasText: searchAddress.addressAssert }).nth(1)).toBeVisible();
