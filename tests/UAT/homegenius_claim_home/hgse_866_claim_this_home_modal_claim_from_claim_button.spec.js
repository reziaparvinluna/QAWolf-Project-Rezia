...
test("hgse_866_claim_this_home_modal_claim_from_claim_button", async () => {
 // Step 1. HGSE-866: Claim This Home Modal. Claim from Claim Button.
  //--------------------------------
  // Arrange:
  //--------------------------------
  // This needs to be an off market property
  const searchAddress = {
    searchAddress: "235 Park Lane, Hendersonville, NC 28791",
    searchAddr2: "235 Park Ln",
    addressLineOne: "235 Park Lane",
    addressLineTwo: "Hendersonville,  NC 28791",
  ...
  await expect(page.locator(`p:has-text("${searchAddress.addressLineOne}") >>nth=0`)).toBeVisible()
  ...
  await expect(page.locator(`div[height="9.688rem"]:has-text("${searchAddress.addressLineOne}")`)).toBeVisible()
