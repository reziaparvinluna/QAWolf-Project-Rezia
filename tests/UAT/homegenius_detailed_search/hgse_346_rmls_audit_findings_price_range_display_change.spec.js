...
test("hgse_346_rmls_audit_findings_price_range_display_change", async () => {
 // Step 1. HGSE-346 - RMLS Audit Findings - Price Range Display Change
  ...
  await page.locator(`a:has-text("Saved Homes")`).click({delay: 3000});
  
  // Assert that the price is the high value
  await expect(page.locator(`p:has-text("${highPrice}")`)).toBeVisible()
 ...
 // Step 2. Assert that geniusprice is higher than the highprice
  ...
  // Unclaim the second address if needed
  await unclaimProperty(page,{searchAddress:searchAddress2,addressLineOne:searchAddress2})
  ...
  // Click on Claim
  await page.locator(`li:has-text("${searchAddressOne}")`).first().click();
  ...
  await page.waitForSelector(`button:text("Skip and Close")`, {timeout: 60_000});
