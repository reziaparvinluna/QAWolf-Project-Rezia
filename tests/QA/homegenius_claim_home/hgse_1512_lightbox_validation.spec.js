...
test("hgse_1512_lightbox_validation", async () => {
 // Step 1. HGSE-1512 - Lightbox Validation
  ...
  await page
    .locator(`[id="DetailsTransactionLine"] + div:has(svg)`)
    .first()
    .click();
  
  // Click on the picture group with 3 photos
  await page
    .locator(`[class*="HistoryRoomConditionImageContainer"]:has-text("3")`)
    .click();
  ...
  // Assert the Header
  await expect(
    page.locator(`h6:has-text("03/06/13  | Sold — $166,900")`),
  ).toBeVisible();
  
  // Assert the verticcal carousl
  await expect(
    page.locator(
      `[class*="VerticalCarouselContainer"] div [direction="vertical"]`,
    ),
  ).toBeVisible();
  
  // Assert Navigation Bar
  await expect(
    page.locator(`[class*="PropertyHistoryPhotoModalHeaderContainer"] nav li`),
  ).toHaveCount(4);
  
  // Assert the first photo is showing
  await expect(
    page.locator(`[direction="horizontal"] div:text("1/3")`),
  ).toBeVisible();
  ...
  await expect(
    page.locator(`[direction="horizontal"] div:text("2/3")`),
  ).toBeVisible();
  await page.locator(`span:text("chevron_right")`).click();
  await expect(
    page.locator(`[direction="horizontal"] div:text("3/3")`),
  ).toBeVisible();
  ...
  await expect(
    page.locator(`p:text("Listed by Century 21 Realty Masters")`),
  ).toBeVisible();
  ...
  await expect(
    page.locator(
      '[class*="TransactionInformationComponentContainer"] div:has-text("Room Condition") + div',
    ),
  ).toHaveScreenshot("roomScore", { maxDiffPixelRatio: 0.01 });
  
  // Assert separation line
  await expect(page.locator('[id="divider"]')).toHaveCSS(
    "border",
  ...
  await expect(
    page.locator(`[id="divider"] + p:text("Objects Identified")`),
  ).toBeVisible();
  await expect(
    page.locator(`[class*="AmenitiesGroup"]:has-text("Driveway: Yes")`),
  ).toBeVisible();
  await expect(
    page.locator(`[class*="AmenitiesGroup"]:has-text("Trees: Yes")`),
  ).toBeVisible();
  await expect(
    page.locator(`[class*="AmenitiesGroup"]:has-text("Flooring: Other")`),
  ).toBeVisible();
  await expect(
    page.locator(`[class*="AmenitiesGroup"]:has-text("Grass: Yes")`),
  ).toBeVisible();
  ...
  await expect(page.locator('button:text("Next Transaction")')).toHaveCSS(
    "background-color",
    "rgb(76, 76, 255)",
  );
  ...
  await expect(
    page.locator(`h6:has-text("01/19/05  | Sold — $245,000")`),
  ).toBeVisible();
  ...
  await expect(page.locator('button:text("Previous Transaction")')).toHaveCSS(
    "background-color",
    "rgb(190, 220, 255)",
  );
  ...
  await expect(page.locator('button:text("Exterior Front")')).toHaveCSS(
    "color",
    "rgb(31, 31, 255)",
  );
  ...
  await expect(
    page.locator(`[direction="horizontal"] div:text("1/1")`),
  ).toBeVisible();
  await expect(
    page.locator(
  ...
  await page
    .locator(
      `[class*="PropertyHistoryPhotoModalHeaderContainer"] button:has-text("close")`,
    )
    .click();
  
  // Assert lightbox is now close and property history section is display
  await expect(
    page.locator(`[class*="PropertyHistoryPhotoModalHeaderContainer"]`),
  ).not.toBeVisible();
