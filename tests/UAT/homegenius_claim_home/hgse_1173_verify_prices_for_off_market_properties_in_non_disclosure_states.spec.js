...
test("hgse_1173_verify_prices_for_off_market_properties_in_non_disclosure_states", async () => {
  ...
  */
  ...
  // Click on the first result with the state
  await page.getByRole(`button`, { name: `${nonState}` }).hover({ force: true });
  await page
    .getByRole(`button`, { name: `${nonState}` })
    .click({ timeout: 5000 });
  ...
  // Grab the address of the third property listed for later assertion
  await page.waitForSelector(`a:has(button:text("Sold on")) >> nth=4`, {
    timeout: 60_000,
  });
  const homeAddress = await page
    .locator(`a:has(button:text("Sold on")) >> nth=4 >> [type="LARGE_CARD"]`)
    .allInnerTexts();
  
  // Click on the second property listed
  await page.locator(`a:has(button:text("Sold on")) >> nth=4`).click();
  ...
  await page
    .locator(`main button span span:text("close")`)
    .last()
    .click({ delay: 5000 });
  ...
  const soldPropertiesCount = await page
    .locator(`button:has-text("Sold on")`)
    .count();
  await expect(
    page.locator(`[class*="card-media-container"] + div:has-text("$--")`),
  ...
  if ((await loader.count()) > 0) {
    await expect(loader.first()).not.toBeVisible({ timeout: 3 * 60 * 1000 });
  }
  ...
  const comparableHomesCount = await page
    .locator(`[class*="SelectablePropCard"]`)
    .count();
  await expect(
    page.locator(`[class*="SelectablePropCard"]:has-text("$--")`),
  ).toHaveCount(comparableHomesCount);
  
  // Assert all pins are also showing $-- on prices
  await expect(page.locator(`div:text("$--")`)).toHaveCount(
    comparableHomesCount * 2,
  );
  
  // Ensure loader not visible
  await expect(page.locator(`.loader`).nth(1)).not.toBeVisible({
    timeout: 4 * 60 * 1000,
  });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await page.locator(`[class*="SelectablePropCard"] >> nth=${i}`).click();
    // Save the home for later Assertion
    const homeInfo = await page
      .locator(`[class*="SelectablePropCard"] >> nth=${i} >> [type="LARGE_CARD"]`)
      .allInnerTexts();
  ...
  // Assert claimed home is showing $-- on price
  const normalizedAddress = homeAddress[1].replace(
    /(Ave|Avenue|St|Street|Rd|Road)(?=[A-Z])/g,
    "$1 ",
  ); // inserts space if missing
  
  const [streetNumber, ...rest] = normalizedAddress.split(" ");
  
  const direction =
    rest.find((part) => /^(N|S|E|W|NW|NE|SW|SE)$/i.test(part)) || "";
  const streetType =
    rest.find((part) => /^(Ave|Avenue|St|Street|Rd|Road)$/i.test(part)) || "";
  const streetName = rest
    .filter((part) => part !== direction && part !== streetType)
    .join(" ");
  
  const streetTypePattern =
    streetType === "Street" || streetType === "St"
      ? "(St|Street)"
      : streetType === "Road" || streetType === "Rd"
        ? "(Rd|Road)"
        : streetType === "Avenue" || streetType === "Ave"
          ? "(Ave|Avenue)"
          : "";
  
  const addressPattern = new RegExp(
    `(?=.*${streetNumber})(?=.*${direction})?(?=.*${streetName})${streetTypePattern ? `(?=.*${streetTypePattern})` : ""}`,
    "i",
  );
  
  // Use locator
  const locator = page.locator('[data-testid="undecorate"]', {
    hasText: addressPattern,
  });
  
  // Assert that "$--" price is visible:
  await expect(
    page.locator(
      `div[type="LARGE_CARD"]:has-text("${streetNumber} ${streetName.slice(0, streetName.length - 4)}") >> .. >> .. >> div:text("$--")`,
    ),
  ).toBeVisible({ timeout: 2_000 });
  
  // Clean Up - unclaim property
  try {
    await page.locator(`div[type="LARGE_CARD"]:has-text("${streetNumber} ${streetName.slice(0, streetName.length - 4)}")`).last().click()
    // Click on the meat icon next to Claimed View
    await page
      .locator(`[aria-label="Claimed property options menu button"]`)
      .click();
