...
export function createFlexibleAddressRegex (address) {
  const pattern = address
    .replace(/\b(Court|Ct)\b/gi, '(Court|Ct)')
    .replace(/\b(Road|Rd)\b/gi, '(Road|Rd)')
    .replace(/\b(Street|St)\b/gi, '(Street|St)')
    .replace(/\b(Avenue|Ave)\b/gi, '(Avenue|Ave)')
    .replace(/\b(Boulevard|Blvd)\b/gi, '(Boulevard|Blvd)')
    .replace(/\b(Drive|Dr)\b/gi, '(Drive|Dr)')
    .replace(/\b(Lane|Ln)\b/gi, '(Lane|Ln)')
