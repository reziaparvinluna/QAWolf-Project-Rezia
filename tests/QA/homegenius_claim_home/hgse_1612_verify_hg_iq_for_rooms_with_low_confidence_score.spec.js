...
test("hgse_1612_verify_hg_iq_for_rooms_with_low_confidence_score", async () => {
 // Step 1. HGSE-1612: Verify hgIQ for Rooms with Low Confidence Score
  ...
  try {
    await page.getByRole(`button`, { name: `Claim Home` }).first().click({ timeout: 5 * 1000});
  } catch(err) {
    console.log(err)
  }
  await expect(page.getByRole(`heading`, { name: `Claim this home` })).toBeVisible();
  ...
  // Select transaction on 06/01/16
  await page.locator(`span:text("06/01/16")`).first().click();
  
  //--------------------------------
  // Assert:
  //--------------------------------
  // Hover on tooltips of low hgIQ and shows warning
  await page.locator(`p:text-is("geniusprice®") + div`).hover();
  await expect(page.getByText(`Automated home price estimate`)).toBeVisible();
