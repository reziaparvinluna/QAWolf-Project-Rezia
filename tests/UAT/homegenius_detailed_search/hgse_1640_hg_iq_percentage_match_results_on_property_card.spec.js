...
test("hgse_1640_hg_iq_percentage_match_results_on_property_card", async () => {
 // Step 1. HGSE-1640: hgIQ Percentage Match Results on Property Card
  ...
  const totalAppliedFilters = 14; // This is the total number of filters (some have 2 filters)
  //--------------------------------
  // Arrange:
  //--------------------------------
  ...
  // await page.locator(`button:has-text("Any") input:visible`).uncheck()
  ...
  await expect(page.locator(
    `button:Has-text("Flooring") + div p:text-is("Bathrooms") + div div[id^="downshift"]:has-text("Wood, Tile")`
  )).toHaveCSS(`border`,`2px solid rgb(31, 31, 255)`)
  await expect(page.locator(
  ...
  while(iterariton<numPages) {
    ...
    for (let i=0; i<propertyCount && i <=numHousesToCheckPerPage; i++) {
      ...
      try{
        ...
        await page2.locator(`.RoomsScrollOverlayWrapper + div p:Has-text("View More")`).click()
      } catch(err){
        console.log(err)
      }
     ...
     // Other Ammenities - Wall Wainscot
      ...
      const matchNumber = Math.ceil((count/totalAppliedFilters)*100)
  ...
  await expect( async () => {
    await page.locator(`button:has-text("chevron_left") + p + button:has-text("chevron_right")`).click({timeout: 1000});
  }).toPass()
 ...
 // Step 2. Removing Filters from Filter by Image removes "Apply" count
  ...
  await expect( async () => {
    await page.getByRole(`button`, { name: `SEARCH WITH AI` }).click({timeout: 1000});
  }).toPass()
