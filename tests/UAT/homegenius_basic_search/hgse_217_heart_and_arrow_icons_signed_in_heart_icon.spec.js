...
test("hgse_217_heart_and_arrow_icons_signed_in_heart_icon", async () => {
 // Step 1. HGSE-217: Heart and Arrow Icons - "Signed In - Heart Icon"
  ...
  await page.locator(`[data-testid="undecorate"]:has-text("$"):not(:has([d*="M12 21.6229"])) >> nth=0`).click()
  
  // Grab the address on the property
  const addressSavedAssert = await page.locator(
  ...
  await expect(page.locator(
    `nav + div button [name="favorite"] + p:text("Save")`
  )).toBeVisible();
  ...
  await expect(page.locator(
    `nav + div button [name="favorite"] >> nth=0`
  ...
  // - Color fill: #FFFFFF
  await expect(page.locator(
    `nav + div button [name="favorite"] >> nth=0 >> g path`
  ...
  await expect(page.locator(
    `[data-testid="undecorate"]:has-text("${addressSavedAssert}")`
  )).toBeVisible();
 ...
 // Step 2. Find Multiple Homes & Use Heart to Save Homes
  ...
  // - Width x Height = 24x24px
  await expect(page.locator(
    `button svg[name="favorite"] >> nth=1`
  )).toHaveAttribute('width', '24')
  await expect(page.locator(
    `button svg[name="favorite"] >> nth=1`
  )).toHaveAttribute('height', '24')
  ...
  // - Color fill: #FFFFFF
  await expect(page.locator(
    `button svg[name="favorite"] >> nth=1 >> g path`
  )).toHaveCSS('color', 'rgb(0, 0, 0)')
  
  // Step 11 
  // Hover over the icon with the mouse and validate that the heart icon turns completely white. Fill color should be #FFFFFF.
  await page.locator(
    `button svg[name="favorite"] >> nth=1`
  ).hover();
  await expect(page.locator(
  ...
  // Step 13 
  // Click on the "Saved Homes" option on the top menu bar.
  const addressSaved = await page.locator(
  ...
  await expect(page.locator('[data-testid="undecorate"]', { hasText: regex })).toBeVisible();
  
  
  // Assert the Heart icon is filled black color
  await expect(
    page.locator('[data-testid="undecorate"]', { hasText: regex }).locator(`span:text("favorite")`)
  ).toHaveCSS('color', 'rgb(0, 0, 0)')
